...
- **Game Passes**: DoubleCoin (2x Coins), VIP (1.5x Coins+XP), ExtraSlot (3rd Slot), DoubleXP (2x XP) — toggle ON/OFF each
...
### ไฟล์ที่เกี่ยวข้อง:
- `src/shared/GamePassLogic.luau` — **Pure business logic** (ownership resolution, multipliers, validation) — no Roblox service deps, fully unit-testable
- `src/server/ShopManager.luau` — Server purchase validation + class gacha + game pass API calls — delegates logic to GamePassLogic
- `src/server/ShopZoneManager.luau` — Zone detection + model placement (InsertService)
- `src/server/ItemManager.luau` — `setItemInSlot()` + `getPlayerSlots()` + `getMaxSlots()` delegates to GamePassLogic
- `src/server/GameManager.luau` — Wire ShopManager + multipliers delegate to GamePassLogic
...
GamePasses = {
    ...
    DoubleXP   = { id = "DoubleXP",   name = "2x XP",    gamePassId = 0,          robuxPrice = 149 },
},
GamePassOrder = { "DoubleCoin", "VIP", "ExtraSlot", "DoubleXP" },
```
...
### GamePassLogic (Shared — Pure Functions):
- `isValidPassId(passId)` — validate passId exists in Config.GamePasses
- `getPassIdByGamePassId(numericId)` — reverse lookup: numeric gamePassId → string key
- `hasGamePass(passesMap, passId)` — pure ownership check (true only if `== true`)
- `resolveOwnership({ savedPasses, apiResults, debugEnabled })` — **core logic**: merge saved + API + debug overrides; priority: debug OFF > API > saved
- `getCoinMultiplier(ownedPasses)` — base 1 + DoubleCoin(+1) + VIP(+0.5)
- `getXPMultiplier(ownedPasses)` — base 1 + DoubleXP(+1) + VIP(+0.5)
- `getMaxItemSlots(ownedPasses)` — 2 default, 3 with ExtraSlot
- `isRateLimited(lastTime, now, cooldown)` — pure time comparison
...
- **Game Pass** (`setupGamePassListeners`): listens to `PromptGamePassPurchaseFinished` → uses `GamePassLogic.getPassIdByGamePassId()` → update cache → fire `GamePassUpdate`
- **Game Pass Check** (`checkAllGamePasses`): calls `queryRobloxAPI()` for API results → delegates to `GamePassLogic.resolveOwnership()` for final state
- **`queryRobloxAPI(player)`**: isolated side-effectful helper — pcall loop over `UserOwnsGamePassAsync()`
- **`hasGamePass(player, passId)`**: delegates to `GamePassLogic.hasGamePass()`
...
| `ToggleTestGamePass` | Client → Server | 🧪 Toggle test gamepass `{ passId, enabled }` (debug mode) |
...
2. `shopManager:checkAllGamePasses()` → loads from profile → `queryRobloxAPI()` → `GamePassLogic.resolveOwnership()` merges all sources
3. `resolveOwnership` priority: debug OFF override > API owned > saved state (saved `false` = explicitly disabled via test toggle)
4. After native Roblox purchase → `PromptGamePassPurchaseFinished` → `GamePassLogic.getPassIdByGamePassId()` → update cache + save
...
**Coin Multiplier** (`GamePassLogic.getCoinMultiplier()`, called by `GameManager:getCoinMultiplier()`) — **additive stacking**:
- Base: ×1
- DoubleCoin: +1.0 → ×2
- VIP: +0.5 → ×1.5
- DoubleCoin + VIP: ×2.5

Applied to: stage clear, stage rewards, finish bonus, coin pickups

**XP Multiplier** (`GamePassLogic.getXPMultiplier()`, called by `GameManager:getXPMultiplier()`) — **additive stacking**:
- Base: ×1
- DoubleXP: +1.0 → ×2
- VIP: +0.5 → ×1.5
- DoubleXP + VIP: ×2.5

Applied inside `GameManager:addMasteryXP()` — affects all XP sources (stage complete + finish)

**ExtraSlot**: `GamePassLogic.getMaxItemSlots()` returns 3 when owned — called by `ItemManager:getMaxSlots()`, server guards + client UI adapts

**VIP Visuals** (applied in `GameManager:applyVIPEffects()`):
- ⭐ VIP Badge: BillboardGui above head (gold gradient, positioned above title)
- [VIP] Chat Tag: client-side `TextChatService.OnIncomingMessage` prepends gold `[VIP]` prefix
- Player attribute: `player:SetAttribute("IsVIP", true)` for client-side reads
- Re-applied on: character spawn, VIP toggle (ShopManager callback)

**Testing Menu toggles**: DoubleCoin, VIP, ExtraSlot, DoubleXP — all under "Game Passes" section in ItemTestingUI
...
| `ItemUI` | กลางล่าง (center-bottom) | 🎯 2-3 Item slots (70×70, horizontal, 3rd with ExtraSlot pass) + Tooltip |
...
| `MatchLobbyUI` | กลางจอ | 🏁 Matchmaking lobby + Rankings — buttons: LEAVE (left) + START MATCH (right), TextLabel pattern for white text |
...
- **CURRENCY EARNED** (breakdown — shows BASE values before multiplier):
  - Coins (X × 1) = +X
  - Stage Clear (X × 5) = +X
  - Stage Rewards = +X
  - Finish Bonus = +25
...
- **Popup size**: 400 × 600 (660 with gamepass upsell)
...
StageTracker:initPlayer() + CurrencyManager:initPlayer() + ItemManager:initPlayer()
    ↓
shopManager:checkAllGamePasses() (async — detects purchased passes immediately)
    ↓
...
Pass Checkpoint → onStageComplete():
  ...
  - CurrencyManager:addCurrency(PerStage × coinMultiplier) ← Stage Clear bonus
  - CurrencyManager:addCurrency(StageReward × coinMultiplier) ← Stage Reward ตามด่าน
  - addMasteryXP(PerStageXP × xpMultiplier) ← Class Mastery XP
    ↓
Touch EndPart of last stage (Finish Line)
    ↓
GameManager:onPlayerFinished() → Set teleportingToLobby flag
    ↓
Give bonuses for LAST stage (all × coinMultiplier):
  - Stage Clear bonus (PerStage × coinMultiplier)
  - Stage Reward (ตามด่านสุดท้าย × coinMultiplier)
  - Finish Bonus (× coinMultiplier)
  - addMasteryXP(FinishBonusXP × xpMultiplier)
  - +1 gem (FinishRace)
...
119. **Game Pass Tab**: 4 passes (2x Coins ID=1740745787, VIP ID=1741108491, Extra Slot ID=1740769746, 2x XP ID=1753504703) — native Roblox purchase via `MarketplaceService:PromptGamePassPurchase()`, server verifies via `UserOwnsGamePassAsync()`
120. **Game Pass Ownership**: cached in `ShopManager.playerGamePasses[player]`, checked on shop open + after purchase; pure logic in `GamePassLogic.luau` (shared module)
121. **GamePassLogic refactor**: Pure business logic extracted to `src/shared/GamePassLogic.luau` — ownership resolution, multiplier calculation, validation, rate limiting; ShopManager/GameManager/ItemManager all delegate to it; 45 unit tests in `GamePassLogic.spec.luau`
...
122. **90 tests / 4 suites**: ClassTypes (16), ItemTypes (18), StageInfo (11), GamePassLogic (45) — pure logic, no mocking needed
