...
│   └── ShopZone           # 🛒 Zone เปิด Shop (Cylinder Neon Cyan, right side X=30, Z=15, radius 6)
...
### Lobby Position:
```
ShopZone: (30, 100.3, 15) — Cylinder วงกลม Neon Cyan หน้าร้าน
Size: 0.5×12×12 (diameter 12), CFrame rotated flat, CanCollide=false
ShopModel: วางที่ Z+10 จาก zone (ร้านอยู่หลัง zone)
```
...
- **START disabled state**: ใช้ `BackgroundColor3 = RGB(40,80,50)` + `BackgroundTransparency=0` แทน transparency — ป้องกันพื้นหลังม่วง (BG_BASE) ทะลุผ่านปุ่ม; enable tween คืนสีเขียวสด `RGB(55,195,95)`
