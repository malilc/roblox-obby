...
│       ├── MenuGridUI.luau      # 📱 2x3 menu button grid (Shop/Classes/Collection/Daily/Help/Settings) — left side, vertically centered
...
|--------|---------|-------------|
| `MenuGridUI` | ซ้ายกลาง (X=10, Y=50%) | 📱 2x3 menu grid: Shop/Classes/Collection/Daily/Help/Settings (172×228) |
| `ScoreUI` | ล่างซ้าย | 💎 Gems (Y=1,-100) + 🚩 Stage (Y=1,-132, race only) + ⏱ Timer |
| `CurrencyUI` | ล่างซ้าย (Y=1,-52) | 💰 แสดงเงิน (gold HUD) |
| `ClassSelectionUI` | กลางจอ (popup, 720×480) | 🎭 เลือก Class (List + Detail Panel layout) |
| `TitleHUDUI` | hidden | 🏷️ replaced by MenuGridUI Titles button |
| `TitleCollectionUI` | กลางจอ (popup, 660×540) | 🏷️ Collection UI — 2 tabs: Titles + Trails (manual equip) |
...
| `ShopUI` | กลางจอ (popup, 780×560) | 🛒 Shop 3-tab: Items + Classes gacha + Game Pass — triggered by ShopZone or MenuGridUI |
...
108. **Collection UI (TitleCollectionUI)**: 2 tabs (Titles + Trails), 660×540, header "⭐ COLLECTION", rarity strips 6px+UICorner(0,3), emoji prefix; trail equip manual (ไม่ auto ตาม class)
